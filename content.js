...
var processor = function() {
	var _date = $(this).find("td.grid-cell.column-1").html();
	var _transaction_type = $(this).find("td.grid-cell.column-2").html();
	var _desc = $(this).find("td.grid-cell.column-3").html();

	//AMOUNT
	var _amount = $(this).find("td.grid-cell.column-4");
	if (_amount.find("span.details").length) {
		var _intAmount = parseFloat(_amount.find("span.details").html().replace(/\s/g, '').replace(',','.'));
	} else {
		var	_intAmount = parseFloat(_amount.html().replace(/\s/g, '').replace(',','.'));

	}
	console.log("amount:" + _intAmount);

	//MCC
	var _mcc = parseInt(_desc.substring(_desc.indexOf("MCC ")+4,_desc.indexOf("MCC ")+8));
	//console.log(_date +  " " + _transaction_type + " " + _desc + " " + _intAmount + " mmc: " + _mcc);
	console.log("MCC:" + _mcc + ":");
 	
 	//MMC agg
 	if (agg.has(_mcc)) {
 		agg.set(_mcc, agg.get(_mcc) + _intAmount );
 	} else {
 		agg.set(_mcc, _intAmount)
 	}

 	//dayAgg
 	 	if (agg.has(_date)) {
 		agg.set(_date, agg.get(_date) + _intAmount );
 	} else {
 		agg.set(_date, _intAmount)
 	}
};
...
chrome.runtime.onMessage.addListener(
  function(request, sender, sendResponse) {
    if( request.message === "clicked_browser_action" ) {
    	...
    	aggText = new Map();
    	dayAgg = new Map();
      $("table.grid.x-acct-operations.column-A-J > tbody > tr").each( processor )
      
	for (const [key, value] of agg.entries()) {
		if (isNaN(key)) {
			aggText.set('No MCC', {value: value, mcc:NaN});
		} else {
			aggText.set(mccMap.get(key), {value: value, mcc:key});
		}
	}

	//create table
	if ($('table.extension_added').length){
		$('table.extension_added').remove();
	}
	var table = $('<table>').addClass("redTable");

	//sort by value
	aggText[Symbol.iterator] = function* () {
	...
	var thead = $('<thead>').append(
									$('<tr>').append($('<td>').text("Amount").css('textAlign','right')).append($('<td>').text("Description").css('padding-left','15px')).append($('<td>').text("MCC"))
								 );
	table.append(thead);

	var tbody = $('<tbody>');

	for (let item of aggText) {
	  var row = $('<tr>').append($('<td>').text($.number(item[1].value,2)).css('textAlign','right')).append($('<td>').text(item[0]).css('padding-left','15px')).append($('<td>').text(item[1].mcc));
	  tbody.append(row);
	}

	table.append(tbody);

	$('div.block.level0.expanded0').append(table);

	//day Table
	if ($('table.date_extension').length){
		$('table.date_extension').remove();
	}
	var table = $('<table>').addClass("date_extension");

	//sort by value
	//dayAgg[Symbol.iterator] = function* () {
    //	yield* [...this.entries()].sort((a, b) => a[1] - b[1]);
	//}

	for (let item of dayAgg) {
	  var row = $('<tr>').append($('<td>').text(item[0])).append($('<td>').text($.number(item[1],2)).css('textAlign','right'));
	  table.append(row);
	}

	$('div.block.level0.expanded0').first().append(table);
